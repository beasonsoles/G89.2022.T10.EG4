...
class PatientJsonStore(JsonStore):
    ...
    def add_item(self, item):
        ...
        patient_found = False
        patient_records = self.find_item_list(item.patient_id, "_VaccinePatientRegister__patient_id")
        for patient_record in patient_records:
            if (patient_record["_VaccinePatientRegister__registration_type"] == item.vaccine_type) and \
                    (patient_record["_VaccinePatientRegister__full_name"] == item.full_name):
                patient_found = True
