...
class TestRequestVacID(unittest.TestCase):
    ...
    def test_parametrized_valid_request_vaccination(self):
        ...
        for patient_id, name_surname, registration_type, phone_number, \
            ...
            with self.subTest(test=comment):
                md5 = my_request.request_vaccination_id(patient_id, name_surname,
                                                        registration_type, phone_number, age)
                self.assertEqual(md5, expected_result)

                with open(patient_store, "r", encoding="utf-8", newline="") as file:
                    data_list = json.load(file)
                found = False
                for patient in data_list:
                    if patient["_VaccinePatientRegister__patient_id"] == patient_id:
                        found = True
    ...
    def test__duplicate_valid_request_vaccination(self):
        ...
        found = False
        for patient in data_list:
            if patient["_VaccinePatientRegister__patient_sys_id"] \
                    == md5:
                found = True
        self.assertTrue(found)
    ...
    def test__add_family_valid_request_vaccination(self):
        ...
        self.assertEqual(md5, "f498f09220649fce1e2e8e523d16d212")
        with open(patient_store, "r", encoding="utf-8", newline="") as file:
            data_list = json.load(file)
            found = 0
            for patient in data_list:
                if patient["_VaccinePatientRegister__patient_id"] \
    ...
    def test_request_vaccination_id_ok(self):
        ...
        md5 = my_request.request_vaccination_id("bb5dbd6f-d8b4-413f-8eb9-dd262cfc54e0",
                                                "Pedro Hernandez", "Regular", "+34123456789", "22")
        self.assertEqual("9bc3dcae6701f7f54d71e36e0df12a59", md5)
